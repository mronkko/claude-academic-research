...
**Do not create a git worktree unless the user explicitly asks for one.** Work in the primary checkout. This reverses the earlier guidance here, which told every session to start non-trivial work in a worktree; the user asked for the reversal on 2026-09-11 after a clear-out of stale ones. "Explicitly asks" means the user says so — a task merely being large, risky, or parallelisable is not a reason to create one, and neither is `superpowers:using-git-worktrees` firing. The isolation was not paying for itself: each worktree needed its own `uv sync --group dev` (`uv.lock` and `.venv/` are gitignored, so a fresh one has no environment and every test fails until synced), `.claude/` is gitignored too so project-local permission settings did not follow, and the leftovers accumulated — several hundred MB of `.venv` in checkouts whose branches had already merged, plus five whose git admin dirs had been pruned out from under them.

Multiple agent instances still work this repo concurrently, now sharing one checkout. Before editing, check what is already in flight — `git status` and the recent commits — so two sessions do not edit the same files at once; that collision, not a broken `main`, is the thing worth avoiding. Expect `main` to move under you mid-task: this session watched another instance land two commits on it, so re-check before assuming the HEAD you started from is still current.
...
- `scripts/pipelines/tag_prefix.py` — the per-review tag namespace. Every tag recording *this review's judgement* is written `<prefix>/<family>:<value>` from a mandatory `TAG_PREFIX` in the project's `screening_config.py`; one library routinely holds several reviews, and without the namespace they share the `abstract:*` family while `merge_duplicate_item` unions tag sets. Tags stating a fact about the *item* (`predatory:flag`, `retracted:flag`, `pdf:*`) stay global on purpose — which is also why the library-wide `enrich_*` scripts need no prefix and take no `--config`. `screening_common` needed no change to support this: `items_with_stage_tag(prefix=…)` and `stage_tag_op(prefix, …)` already took a full prefix string, so the stage constants simply became suffixes (`STAGE_TAG_FAMILY` / `STAGE_TAG_SUFFIX`). The `SLR Coding` child note is namespaced through `zotero_io.slr_coding_marker(ns)` for the same reason — one fixed marker meant a second review's coding silently overwrote the first's. `scripts/setup/set_tag_prefix.py` duplicates the validation rules because `scripts/setup/` must stay stdlib-only and cannot import from `scripts/pipelines/`; `tests/unit/test_set_tag_prefix.py` fails if the two copies ever disagree.
