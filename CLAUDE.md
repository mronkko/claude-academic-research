...
- **Skill pre-flight and bootstrap helpers**: when a skill needs to probe config / scaffold / deny-rules, or create a project-local directory, invoke the cross-platform scripts in `scripts/setup/` (`check_configured.py`, `check_project_scaffold.py FILE...`, `check_deny_rules.py RULE...`, `ensure_dir.py DIR...`). Do not use POSIX `test -f` / `mkdir -p`, and do not use inline `python -c`. None of those are covered by the wizard's `Bash(python3 ${CLAUDE_PLUGIN_ROOT}/scripts/**)` allow rule, so they trigger a permission prompt at skill load time; the script paths are covered.
