...
Create the iteration working directory up front:

```bash
mkdir -p /tmp/critic-loop
```
...
while iter <= MAX_ITER:
  ...
  Snapshot: copy the rendered .md to /tmp/critic-loop/iter-{N}/rendered.md.
  ...
  Save each Agent's returned text to /tmp/critic-loop/iter-{N}/critic-<name>.md.
  ...
  Write everything to /tmp/critic-loop/iter-{N}/decisions.md (format below).
...
/tmp/critic-loop/iter-{N}/rendered.md.
...
After the loop exits, write `/tmp/critic-loop/final-report.md`:
...
reports into the chat — point to the files under `/tmp/critic-loop/`.
